...
void Initialize() {
    ...
    displayWindow = SDL_CreateWindow("Run Boy Run", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, SDL_WINDOW_OPENGL);
    ...
    Mix_VolumeChunk(successSound, MIX_MAX_VOLUME);
    failureSound = Mix_LoadWAV("failure.wav");
    Mix_VolumeChunk(failureSound, MIX_MAX_VOLUME);
}
...
void ProcessInput()
{
    ...
    {
        currentScene->state.player->velocity.z = cos(glm::radians(currentScene->state.player->rotation.y)) * -1.0f * currentScene->state.player->speed;
        currentScene->state.player->velocity.x = sin(glm::radians(currentScene->state.player->rotation.y)) * -1.0f * currentScene->state.player->speed;
    }
...
void Render() {
    ...
    if (currentScene->state.failure)
    {
       ...
       program.SetViewMatrix(uiViewMatrix);
        ...
        Util::DrawText(&program, currentScene->state.fontTextureID, "You Lost!", .5f, -.25f, glm::vec3(-1, 0, 0));
    ...
    else if (currentScene->state.success)
    {
        if (!soundPlayed)
        {
        ...
        Util::DrawText(&program, currentScene->state.fontTextureID, "You Won!", .5f, -.25f, glm::vec3(-1, 0, 0));
        currentScene->state.player->speed = 0;
        currentScene->state.player->acceleration = glm::vec3(0);
        currentScene->state.player->velocity = glm::vec3(0);
